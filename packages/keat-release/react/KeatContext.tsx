...
import { createContext, PropsWithChildren, useMemo, useContext, useEffect } from 'react';
import { App, getKeatConfig } from '../src/api';
...
export function KeatProvider({ children }: PropsWithChildren<{}>) {
    ...
    const [rule, setRule] = React.useState<string | number | boolean | undefined>();
    ...
    const context = useMemo((): KeatCtx => {
        return {
            ...
            setRule,
            reset: () => {
                setScreen(localStorage.getItem('__keat_key') ? 'browse' : 'connect');
                setFeature(undefined);
            }
        };
    }, [app, setApp, feature, setFeature, screen, setScreen]);

    return <KeatContext.Provider value={context}>{children}</KeatContext.Provider>;
}
...
export function useKeatContext(): KeatCtx {
    ...
    if (!ctx) {
        throw new Error('Uninitialised: Did you forget to use KeatContext.Provider?');
    }

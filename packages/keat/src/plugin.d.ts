...
export type OnPluginInitHook = (ctx: OnPluginInitCtx, api: OnPluginInitApi) => void | Promise<void>;
...
export type OnPluginIdentifyHook = (ctx: OnPluginIdentifyCtx) => void | Promise<void>;
...
export type PostEvalCtx = EvalCtx & {
    result: unknown;
};
export type OnEvalApi = {
    setUser: (newUser: unknown) => void;
};
export type onPostEvaluateHook = (ctx: EvalCtx & {
    result: unknown;
}) => void;
...
type InferLiteralType<M extends Matcher<any>> = M extends Matcher<infer T> ? T : any;
export declare function createPlugin<M extends Matcher>(plugin: Plugin<M>): Plugin<M>;
export declare function createNopPlugin(): Plugin<Matcher<void>>;
export {};

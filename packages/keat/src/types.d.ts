...
export type User = CustomTypes extends {
    user: infer T;
} ? T : ({
    id: string;
} | {
...
export type KeatApi<TFeatures extends AnyFeatures> = {
    ...
    variation<TFeature extends keyof TFeatures>(feature: TFeature, user?: User, display?: Display): TFeatures[TFeature] extends {
        variates: any;
    } ? TFeatures[TFeature]["variates"] extends readonly any[] ? TFeatures[TFeature]["variates"][number] : boolean : boolean;

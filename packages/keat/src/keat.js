import { load } from "./display";
import { getRules, getVariatesMap, isLiteral } from "./utils";
export function keatCore({ features, display = "swap", plugins = [], }) {
    const names = Object.keys(features);
    const variatesMap = getVariatesMap(features);
    let defaultDisplay = display;
    let defaultUser = undefined;
    let configId = 0;
    let config = {};
    const setConfig = (newConfig) => {
        configId += 1;
        config = newConfig ?? {};
    };
    let listeners = [];
    const handleChange = () => {
        listeners.forEach((l) => l(config));
    };
    const initPromise = Promise.allSettled(plugins.map((p) => p.onPluginInit?.({ features: names, variates: variatesMap }, {
        setConfig,
        onChange: handleChange,
    }))).then(() => {
        if (configId === 0)
            return;
        handleChange();
    });
    let loader = load(initPromise);
    const evaluate = (feature, user, configId) => {
        const variates = variatesMap[feature];
        const rules = getRules(features, config, feature, configId);
        if (!rules)
            return variates[variates.length - 1];
        let result;
        let ctx = {
            feature,
            variates,
            rules,
            user,
            configId,
        };
        const preApi = {
            setUser: (newUser) => {
                ctx.user = newUser;
            },
        };
        plugins.forEach((p) => p.onPreEvaluate?.(ctx, preApi));
        for (let i = 0; i < variates.length; i++) {
            const variate = variates[i];
            const rule = rules[i];
            const ok = evaluateVariate({ ...ctx, variate }, plugins, rule);
            if (ok) {
                result = variate;
                break;
            }
        }
        if (result === undefined) {
            const index = rules.findIndex((v) => v === true) ?? -1;
            result = index === -1 ? variates[variates.length - 1] : variates[index];
        }
        plugins.forEach((p) => p.onPostEvaluate?.({ ...ctx, result }));
        return result;
    };
    return {
        ready: (display = defaultDisplay) => loader.ready(display),
        configure: (newConfig) => {
            setConfig(newConfig);
            handleChange();
        },
        identify: (user, noReload) => {
            defaultUser = user;
            plugins.forEach((p) => p.onIdentify);
            if (noReload)
                return;
            const currentId = configId;
            loader = load(Promise.allSettled(plugins.map((p) => p.onIdentify?.({ user }))).then(() => {
                if (configId === currentId)
                    return;
                handleChange();
            }));
        },
        setDisplay: (display) => (defaultDisplay = display),
        variation: (feature, user = defaultUser, display = defaultDisplay) => {
            const useLatest = loader.useLatest(display);
            return evaluate(feature, user, useLatest ? configId : 0);
        },
        onChange: (listener) => {
            listeners.push(listener);
            return () => {
                listeners = listeners.filter((l) => l === listener);
            };
        },
    };
}
function evaluateVariate(ctx, plugins, rule) {
    if (rule === undefined)
        return false;
    return isLiteral(rule)
        ? plugins.some((p) => {
            const matchers = Array.isArray(p.matcher) ? p.matcher : [p.matcher];
            for (const matcher of matchers) {
                const literal = matcher(rule);
                if (literal === null)
                    continue;
                return p.evaluate?.({ ...ctx, literal }) ?? false;
            }
        })
        : rule.OR.some((r) => evaluateVariate(ctx, plugins, r));
}
